...
- 📂 **目录直挂** — 指定已有图片 / 视频目录只建索引，不复制原文件；可选监视目录变化
...
左侧「导入素材」→「添加素材目录」，扫描已有图片和视频目录。素材保留在原目录，应用只保存索引、文件大小 / 修改时间和缩略图缓存（缩略图缓存在应用数据目录，不污染源目录）。

- **源文件缺失** — 目录断开或文件移走时，卡片标记「源文件缺失」；可在「素材目录」中校验、重新定位、增量扫描或移除挂载；重新定位后按原相对路径批量恢复
- **监视此目录** — 每个挂载目录可单独开启，**默认关闭**。开启后新增媒体自动建立外链索引；删除只标记缺失，不删索引、不删原文件；可唯一识别的重命名会更新相对路径，无法识别时按「旧条目缺失 + 新条目新增」处理；关闭开关后不再自动同步
- **分享与压缩** — ZIP 分享只读外链源文件并把媒体写入分享包；外链素材不参与应用内压缩 / 转码，也不会被原地覆盖；如需压缩，请先用「导入图片」创建托管（managed）副本
...
3. **导入素材** 左侧「导入素材」：图片、粘贴、文档或分享包都行  
   ...
   - 若已有图库目录：可用「添加素材目录」做 [目录直挂](#目录直挂)（只建索引、不复制原文件；需要自动同步时在「素材目录」开启「监视此目录」）
...
<details>
<summary><b>可以把已有图片目录直接挂进来吗？会不会复制一份？</b></summary>

**A：** 可以。左侧「导入素材」→「添加素材目录」后只建立索引，**不复制**原始图片 / 视频；缩略图缓存在应用数据目录。删除库内条目只会去掉索引和缓存，**不会删除**原目录里的文件。详见上方 [目录直挂](#目录直挂)。

</details>

<details>
<summary><b>目录里的图被移走或重命名了怎么办？</b></summary>

**A：** 卡片会显示「源文件缺失」。到「素材目录」做校验、重新定位目录或增量扫描；重新定位后按原相对路径恢复。若开启了「监视此目录」，新增会自动入库，删除默认只标缺失；能唯一匹配的重命名会更新相对路径。
